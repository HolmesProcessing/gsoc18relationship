...
objdump_results = objdump_rdd.map(lambda x: (x.sha256,
                                             x.service_name,
                                             find_op_in_objdump(x.results),
                                             convert_to_labels(x.source_tags),
                                             int(time.time())
                                            ) if 'benign' not in x.source_tags else None
                                 ).filter(bool)
objdump_objects = objdump_rdd.map(lambda x: (x.sha256,
                                             find_op_in_objdump(x.results),
                                             convert_to_labels(x.source_tags),
                                             int(time.time())
                                            ) if 'benign' not in x.source_tags else None
