...
cuckoo_results = cuckoo_rdd.map(lambda x: (x.sha256, x.service_name, find_api_call_in_cuckoo(x.results), convert_to_labels(x.source_tags), int(time.time())) if 'benign' not in x.source_tags else None).filter(bool)
cuckoo_objects = cuckoo_rdd.map(lambda x: (x.sha256, find_api_call_in_cuckoo(x.results), convert_to_labels(x.source_tags), int(time.time())) if 'benign' not in x.source_tags else None).filter(bool)
...
cuckoo_df = cuckoo_df.withColumnRenamed("_1", "sha256").withColumnRenamed("_2", "service_name").withColumnRenamed("_3", "features").withColumnRenamed("_4", "labels").withColumnRenamed("_5", "timestamp")
...
cuckoo_df = cuckoo_df.withColumnRenamed("_1", "sha256").withColumnRenamed("_2", "features_cuckoo").withColumnRenamed("_3", "labels").withColumnRenamed("_4", "timestamp")
